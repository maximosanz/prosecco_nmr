...
def _get_seq_fromfasta(ls):
	...
	for i,l in enumerate(ls):
		if not l.strip():
			continue
		elif seqid is None and l[0] != ">":
			seqid = "Unnamed_sequence"
		if l[0] == ">":
			if seqid is not None:
				Seqs.append((seqid,seq))
				seq = ''
				_get_seq_fromfasta(ls[i:])
			seqid = l[1:].strip()
			continue
		seq += l.strip()
	Seqs.append((seqid,seq))
...
def parse_fasta(fastafn):
	f = open(fastafn)
	return _get_seq_fromfasta(f.readlines())


def input_fromseq(seq, **kwargs):

	default_kwargs={ 
	'job_name' : "prosecco_job",
	'NN_type' : "fully_connected", # NLP parsing not yet implemented here
	'useBLOSUM' : True,
	'seq_neigh' : 2,
	'SS_neigh' : 3,
	'SS_type' : "PSIPRED", # DSSP needs to be implemented
	...
	seq_Arr = _extract_sequence_array(seq_raw,useBLOSUM=kw['useBLOSUM'])
	special_Arr = _specialRes_fromseq(seq)
	# DSSP needs to be implemented
	if kw['ss2_file'] is not None:
		SS_seq, SS_arr = parse_ss2(kw['ss2_file'])
		if SS_seq != seq_raw:
			raise ValueError("Sequence in ss2 file {} is different from input sequence".format(kw['ss2_file']))
	else:
		SS_arr = _PSIPRED_fromseq(seq_raw,kw['job_name'],psipred_exe=kw['psipred_exe'])
	seq_special_Arr = np.concatenate([seq_Arr,special_Arr],axis=1)

	Input = []
	for pos in range(len(seq)):
		seq_Input = _extract_window(pos,seq_special_Arr,kw['seq_neigh'])
		SS_Input = _extract_window(pos,SS_arr,kw['SS_neigh'],add_termini=True)
